...
export function toFullSKU(econItem: EconItem): string {
	...
	if (parts != null) {
		...
		for (const part of parts)
			partsDefindices.push(getPartDefindex(`Strange Part: ${part}`));
	...
	if (spells != null) {
		...
		for (const spell of spells)
			spellsDefindices.push(getSpellEnum(`Halloween Spell: ${spell}`));
		spellsDefindices.sort();
		for (const spellDefindex of spellsDefindices)
			attachToSKU(`${SKUPrefix.Spell}${spellDefindex}`);
	}

	if (sheen != null) attachToSKU(`${SKUPrefix.Sheen}${getSheenEnum(sheen)}`);
	if (killstreaker != null)
		attachToSKU(
			`${SKUPrefix.Killstreaker}${getKillstreakerEnum(killstreaker)}`
		);
