...
export function toBaseFromParsedEcon(parsedEconItem: ParsedEconItem, econItem: EconItem): string {
  // fallback defindex
  ...
  if (parsedEconItem.quality == null) {
    const extractedQuality =
      econItem.tags.find((tag) => tag.category == "Quality")?.internal_name || null
